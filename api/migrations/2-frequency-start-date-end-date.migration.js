...
// Also: for weekly goals (`total_week_frequency`), ensure the `start_date` lies on a Monday, else
// set it to the first Monday after the `frequency_change_date`.
...
function getFirstMondayAfterDate(date) {
  ...
  let numDaysToAdd = (8 - date.getUTCDay()) % 7;  // 1 for Sunday, 0 for Monday, 6 for Tuesday, ..., 2 for Saturday
...
db.habits.find().snapshot().forEach(habit => {
  ...
  if (frequency.type_name === "total_week_frequency") {
    // the goal is a weekly goal
